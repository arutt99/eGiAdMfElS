...
function status() {
  ...
  if (game.choice) {
    const text = { hook: ['Replay your loot', 'Drag a highlighted card from your hand onto the table.'], sword: ['Take what’s missing', 'Drag a highlighted card from Rook’s hand onto the table.'], cannon: ['Fire the cannon', 'Swipe a highlighted card from Rook’s hand to the right.'], map: ['Follow the map', 'Choose one of the recovered cards on the right.'] };
...
function render() {
  ...
  const dock = game.phase === 'over' ? '<button class="primary" data-action="results">View final scores <span>→</span></button>'
    ...
    : choice ? '<div class="choice-reminder floating">Choose one of the cards floating on the right.</div>'
