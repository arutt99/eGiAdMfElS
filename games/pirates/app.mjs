...
function bankMarkup(n) {
  ...
  return `<section class="bank ${n === game.active && game.phase === 'play' ? 'active-bank' : ''}" aria-label="${playerName(n)} bank"><div class="section-heading"><span>${n === 0 ? 'Your treasure' : 'Rook’s treasure'}</span><button class="text-button" data-action="bank" data-player="${n}">${bank.length} cards <span aria-hidden="true">↗</span></button></div><div class="bank-grid">${SUITS.map(s => {
    const cards = stack(bank, s.id), top = cards[0], can = eligible.includes(top?.id);
    return `<button class="bank-slot ${top ? 'filled' : ''} ${can ? 'target' : ''}" style="--suit:${s.color}" data-action="${can ? 'choose' : 'stack'}" data-player="${n}" data-suit="${s.id}" data-id="${top?.id || ''}" aria-label="${s.name}: ${top ? cards.map(c => c.value).join(', ') : 'empty'}${can ? ', choose this card' : ''}">${icon(s.id)}<b>${top?.value || '–'}</b><span>${s.name}</span>${cards.length > 1 ? `<i>${cards.length}</i>` : ''}</button>`;
  }).join('')}</div></section>`;
}
...
function home() {
  screen = 'home';
  $('#app').innerHTML = `<div class="shell home">${topbar(true)}<section class="landing"><p class="eyebrow">FORTUNE FAVORS THE BOLD</p><div class="hero-art"><span class="orbit orbit-one"></span><span class="orbit orbit-two"></span><span class="north">N</span>${icon('ship', 'hero-ship')}<span class="sea sea-one"></span><span class="sea sea-two"></span><span class="spark spark-one">✦</span><span class="spark spark-two">✧</span></div><h1>Pirates<span>A little greed.<br>A lot to lose.</span></h1><p class="intro">Draw treasure. Outsmart your rival.<br>Know when to sail away.</p><div class="voyage-meta"><span>YOU vs. CAPTAIN ROOK</span><span>10–15 MIN</span></div><button class="primary start-button" data-action="${saved && saved.phase !== 'over' ? 'resume' : 'start'}">${saved && saved.phase !== 'over' ? 'Continue voyage' : 'Set sail'} <span>→</span></button>${saved && saved.phase !== 'over' ? '<button class="text-button new-voyage" data-action="restart">Start a new voyage</button>' : ''}<button class="secondary learn-button" data-action="help">Learn the ropes</button><p class="save-note">One rival. Ten suits. One chance to get greedy.</p></section><footer class="home-footer">A GAME OF NERVE & NAUTICAL FORTUNE</footer></div>`;
}
function status() {
  ...
  if (game.choice) {
    const text = { hook: ['Choose your loot', 'Hook: replay a top card from your own bank.'], sword: ['Take what’s missing', 'Sword: steal a top card of a suit absent from your bank.'], cannon: ['Pick a target', 'Cannon: discard one of Rook’s top cards.'], map: ['Follow the map', 'Choose one recovered card to play.'] };
...
function move(action) {
  ...
  scrollFrame = requestAnimationFrame(() => {
    if (game.active === 0 && game.choice) $('.choice-panel')?.scrollIntoView({ block: 'center', behavior: 'smooth' });
