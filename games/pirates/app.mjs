import { SUITS, suitOf, suitCopy, traitOf, traitCopy, traitPool, newGame, act, options, choiceZone, protectedIds, scoreOf, stack, gain, duplicate, botAction, validSave } from './engine.mjs';
import { icon } from './icons.mjs';

const $ = selector => document.querySelector(selector);
const SAVE_KEY = 'pirates.v1';
const VARIANT_KEY = 'pirates.variants';
const VARIANTS = [
  { id: 'traits', name: 'Traits', suit: 'chest', short: 'Unique captain powers', blurb: 'Each captain draws two trait cards, keeps one and sails with that power for the whole voyage.' },
  { id: 'mermaid', name: 'Mermaid', suit: 'mermaid', short: 'Sirens gain an ability', blurb: 'Mermaids drop to 2–7 like every other suit and gain an ability: replay a card already in your play area.' },
];
let game = null, saved = null, timer, fast = false, screen = 'home', saveFailed = false, scrollFrame;
let variants = { traits: false, mermaid: false };
try {
  const data = JSON.parse(localStorage.getItem(SAVE_KEY)); if (validSave(data)) saved = data;
  fast = localStorage.getItem('pirates.fast') === 'true';
  const picked = JSON.parse(localStorage.getItem(VARIANT_KEY));
  if (picked) variants = { traits: Boolean(picked.traits), mermaid: Boolean(picked.mermaid) };
} catch { /* Storage can be unavailable in private browsers. */ }
const esc = value => String(value).replace(/[&<>"']/g, c => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[c]);
const playerName = n => n === 0 ? 'You' : 'Captain Rook';

function persist() {
  saved = game;
  try { localStorage.setItem(SAVE_KEY, JSON.stringify(game)); } catch { saveFailed = true; }
}
function smallCard(c, { action = 'suit', danger = false, protectedCard = false, target = false, caption = '' } = {}) {
  const s = suitOf(c);
  return `<button class="loot-card ${danger ? 'danger' : ''} ${protectedCard ? 'protected' : ''} ${target ? 'target' : ''}" style="--suit:${s.color};--card-art:url('assets/cards/${s.id}.jpg')" data-action="${action}" data-id="${c.id}" data-suit="${c.suit}" aria-label="${s.name} ${c.value}${danger ? ', causes a bust' : ''}${protectedCard ? ', safe from the bust' : ''}${target ? ', tap to choose this card' : ''}"><span class="card-value">${c.value}</span><span class="card-art" aria-hidden="true"></span><span class="card-name">${s.name}</span>${caption ? `<span class="card-caption">${caption}</span>` : ''}${protectedCard ? '<span class="protected-dot" title="Protected">•</span>' : ''}</button>`;
}
function bankMarkup(n) {
  const bank = game.banks[n];
  // Misfire and Captain's Hook change which hand a pending ability draws from.
  const zone = game.active === 0 ? choiceZone(game) : null;
  const selectable = zone?.zone === 'bank' && zone.player === n;
  const eligible = selectable ? options(game).map(c => c.id) : [];
  return `<section data-bank="${n}" class="bank bank--${n === 0 ? 'player' : 'rival'} ${n === game.active && game.phase === 'play' ? 'active-bank' : ''}" aria-label="${playerName(n)} bank"><div class="section-heading"><span>${n === 0 ? 'Your treasure' : 'Rook’s treasure'}</span>${traitTag(n)}<button class="text-button" data-action="bank" data-player="${n}">${bank.length} cards <span aria-hidden="true">↗</span></button></div><div class="bank-grid">${SUITS.map((s, index) => {
    const cards = stack(bank, s.id), top = cards[0], can = eligible.includes(top?.id);
    const left = index * 100 / 9, shift = -index * 100 / 9, lift = Math.round((1 - Math.abs(index - 4.5) / 4.5) * 8), layer = index + 1;
    return `<button class="bank-slot ${top ? 'filled' : ''} ${can ? 'target drag-source' : ''}" style="--suit:${s.color};--card-art:url('assets/cards/${s.id}.jpg');--hand-left:${left}%;--hand-shift:${shift}%;--hand-tilt:${(index - 4.5) * 1.25}deg;--hand-lift:-${lift}px;--hand-layer:${layer}" data-action="${can ? 'choose' : 'stack'}" data-drag-card="${can ? 'true' : 'false'}" data-player="${n}" data-suit="${s.id}" data-id="${top?.id || ''}" aria-grabbed="false" aria-label="${s.name}: ${top ? cards.map(c => c.value).join(', ') : 'empty'}${can ? ', drag this card to continue' : ''}"><span class="bank-art" aria-hidden="true"></span><b>${top?.value || '–'}</b><span>${s.name}</span>${cards.length > 1 ? `<i>${cards.length}</i>` : ''}</button>`;
  }).join('')}</div></section>`;
}
function topbar(home = false) {
  return `<header class="topbar"><a class="shelf-link" href="../../" aria-label="Back to game shelf">←</a><span class="brand">${icon('ship')} Pirates</span><button class="round-button" data-action="${home ? 'help' : 'menu'}" aria-label="${home ? 'How to play' : 'Game menu'}">${home ? '?' : '☰'}</button></header>`;
}
// Variants are chosen before a voyage sails, and only apply to a new deal.
function variantPicker() {
  return `<div class="variant-picker" role="group" aria-label="Optional variants">${VARIANTS.map(v => `<button class="variant-toggle ${variants[v.id] ? 'on' : ''}" data-action="variant" data-variant="${v.id}" role="switch" aria-checked="${variants[v.id]}"><span class="variant-mark" aria-hidden="true">${variants[v.id] ? '✓' : ''}</span>${icon(v.suit)}<span><b>${v.name}</b><small>${v.short}</small></span></button>`).join('')}</div>`;
}
function home() {
  screen = 'home';
  const resume = saved && saved.phase !== 'over';
  const changed = resume && VARIANTS.some(v => Boolean(saved.variants?.[v.id]) !== variants[v.id]);
  $('#app').innerHTML = `<div class="shell home">${topbar(true)}<section class="landing"><div class="title-lockup"><p class="eyebrow">A GAME OF CHANCE & CUNNING</p><h1>Pirates</h1><p class="intro">Fortune favors the bold.</p></div><span class="match-versus" aria-hidden="true">VS</span><div class="match-banner"><span class="captain-name">Player 1<small>The daring duelist</small></span><button class="primary start-button" data-action="${resume ? 'resume' : 'start'}">${resume ? 'Continue' : 'Play'}<span aria-hidden="true">›</span></button><span class="captain-name">Captain Rook<small>The ruthless rival</small></span></div><div class="landing-tools">${variantPicker()}<button class="secondary learn-button" data-action="help">Learn the ropes</button>${resume ? `<button class="text-button new-voyage" data-action="restart">Start a new voyage${changed ? ' with these variants' : ''}</button>` : '<p class="save-note">Draw your fortune. Know when to walk away.</p>'}</div></section><footer class="home-footer">TEN SUITS · TWO CAPTAINS · ONE FORTUNE</footer></div>`;
}
function traitPick() {
  screen = 'game';
  clearTimeout(timer);
  const offered = game.offer[0].map(traitOf);
  $('#app').innerHTML = `<div class="shell draft-shell">${topbar()}<section class="turn-status"><h2>Choose your trait</h2><p>Two trait cards are dealt to you. Keep one for the whole voyage; Captain Rook keeps one of their own.</p></section><section class="trait-draft" aria-label="Trait cards on offer">${offered.map(t => `<button class="trait-card" data-action="trait" data-id="${t.id}" style="--suit:${SUITS.find(s => s.id === t.suit).color}">${icon(t.suit)}<span><b>${t.name}</b><small>${traitCopy(t, game.variants)}</small></span><i aria-hidden="true">Keep this trait ›</i></button>`).join('')}</section><p class="muted trait-note">The trait you leave behind goes back in the box. Rook reveals theirs as soon as you choose.</p></div>`;
  $('#announcer').textContent = 'Choose your trait to begin the voyage.';
}
function traitTag(n) {
  const t = game.variants?.traits ? traitOf(game.traits[n]) : null;
  if (!t) return '';
  return `<button class="trait-tag" data-action="trait-info" data-id="${t.id}" style="--suit:${SUITS.find(s => s.id === t.suit).color}" aria-label="${n === 0 ? 'Your' : 'Rook’s'} trait: ${t.name}. Tap to read it."><span class="trait-mark" aria-hidden="true">${icon(t.suit)}</span><b>${t.name}</b></button>`;
}
function traitBar() {
  if (!game.variants?.traits) return '';
  return `<div class="trait-bar">${[0, 1].map(n => {
    const t = traitOf(game.traits[n]);
    return `<button class="trait-chip ${n === 1 ? 'rival' : ''}" data-action="trait-info" data-id="${t?.id || ''}" ${t ? '' : 'disabled'} style="--suit:${t ? SUITS.find(s => s.id === t.suit).color : 'var(--muted)'}">${t ? icon(t.suit) : ''}<span><small>${n === 0 ? 'YOUR TRAIT' : 'ROOK’S TRAIT'}</small><b>${t ? t.name : 'Not chosen'}</b></span></button>`;
  }).join('')}</div>`;
}
function status() {
  if (game.phase === 'over') return ['Voyage complete', 'The final treasure has been counted.'];
  if (game.phase === 'turnEnd') return [game.result.busted ? 'Overboard!' : 'Treasure secured', game.message];
  if (game.active === 1) return ['Rook is at the helm', game.choice ? `Resolving ${game.choice.type}…` : game.forced ? `${game.forced} more ${game.forced === 1 ? 'card' : 'cards'} required by the Kraken.` : game.message];
  if (game.choice) {
    const mine = game.traits?.[0], theirs = game.traits?.[1];
    if (game.choice.type === 'mermaid') {
      if (theirs === 'siren') return ['The Siren listens', 'Rook banks whichever card you name. Tap the one you can spare.'];
      if (mine === 'casanova') return ['Casanova’s charm', 'Tap a card already in play to send it straight to your bank.'];
      return ['Sing to the deep', 'Tap a card already in play to move it after the Mermaid and fire its ability again.'];
    }
    const text = {
      hook: [mine === 'captains-hook' && game.choice.repeat ? 'Replay two cards' : 'Replay your loot', 'Drag a highlighted card from your hand onto the table.'],
      sword: [theirs === 'parry' ? 'Parried — take a Kraken' : 'Take what’s missing', 'Drag a highlighted card from Rook’s hand onto the table.'],
      cannon: theirs === 'misfire' ? ['The cannon misfires', 'Swipe a highlighted card from your own hand to the right.'] : ['Fire the cannon', 'Swipe a highlighted card from Rook’s hand to the right.'],
      map: [mine === 'navigator' ? 'Chart the whole discard' : 'Follow the map', options(game).length > 3 ? 'Choose any one card from the strip along the bottom.' : 'Choose one of the recovered cards on the right.'],
    };
    return text[game.choice.type];
  }
  if (game.forced) return ['The Kraken demands more', `Play ${game.forced} more ${game.forced === 1 ? 'card' : 'cards'} before collecting.`];
  if (!game.play.length) return ['Your move, Captain', 'Draw your first card to begin.'];
  if (!game.deck.length) return ['The last of the loot', 'Collect your treasure to finish the voyage.'];
  return ['Press on or sail away?', 'A repeated suit busts. Collect to keep your loot.'];
}
function render() {
  if (game.phase === 'traitPick') { traitPick(); return; }
  screen = 'game';
  const [title, subtitle] = status(), cards = game.result?.cards || game.play;
  const safe = new Set(game.result ? game.result.protectedIds : protectedIds(game));
  const shielded = cards.filter(c => safe.has(c.id)).length;
  const combo = cards.some(c => c.suit === 'key') && cards.some(c => c.suit === 'chest');
  const human = game.active === 0, playing = game.phase === 'play';
  const mine = game.traits?.[0] || null, theirs = game.traits?.[1] || null;
  const preview = gain(game.banks[game.active], game.play, game.traits?.[game.active]);
  const last = cards.at(-1), s = last && suitOf(last);
  const choice = human && game.choice;
  const focus = document.activeElement?.dataset?.focus;
  const zone = choice ? choiceZone(game) : null;
  const bankChoice = zone?.zone === 'bank';
  const playDrop = bankChoice && game.choice.type !== 'cannon';
  const cannonDrop = bankChoice && game.choice.type === 'cannon';
  // The Mermaid ability picks from the table itself, so the haul becomes the target.
  const pickable = zone?.zone === 'play' ? options(game).map(c => c.id) : [];
  const replayCaption = theirs === 'siren' ? 'Rook takes it' : mine === 'casanova' ? 'Bank it' : 'Replay it';
  const deckDisabled = !human || !playing || Boolean(game.choice) || !game.deck.length;
  const dragCopy = { hook: 'Drag it anywhere onto the table.', sword: 'Drag it anywhere onto the table.', cannon: 'Swipe it out to the right of the screen.' };
  const dropLabel = { hook: 'Replay your loot', sword: 'Steal the card', cannon: 'Sink the card' };
  const peek = game.oracle && game.deck.length && playing ? game.deck.slice(-Math.min(game.peek || 1, game.deck.length)).reverse() : [];
  const suitLine = s && suitCopy(s, game.variants);
  const info = `${bankChoice ? `<div class="ability-instruction"><span class="instruction-icon">${icon(game.choice.type)}</span><span><b>${dropLabel[game.choice.type]}</b><small>${dragCopy[game.choice.type]}</small></span></div>` : last ? `<button class="ability-note" data-action="suit" data-suit="${s.id}" style="--suit:${s.color}">${icon(s.id)}<span><b>${s.name}</b> ${suitLine.short}</span><span class="info-mark">i</span></button>` : ''}${peek.map((c, i) => `<div class="oracle-peek" style="--suit:${suitOf(c).color}">${icon('oracle')}<span>${peek.length > 1 ? `Card ${i + 1}` : 'Next card'} <b>${suitOf(c).name} ${c.value}</b></span><strong class="${duplicate(game, c) ? 'risk-text' : ''}">${duplicate(game, c) ? 'Will bust' : 'New suit'}</strong></div>`).join('')}<div class="table-tags">${shielded ? `<span class="tag safe">${shielded} safe from a bust</span>` : ''}${combo ? `<span class="tag gold">Key + Chest bonus ready${mine === 'treasure-hunter' && human ? ' (doubled)' : ''}</span>` : ''}${game.forced && playing ? `<span class="tag warning">${game.forced} more required</span>` : ''}</div>`;
  const dock = game.phase === 'over' ? '<button class="primary" data-action="results">View final scores <span>→</span></button>'
    : game.phase === 'turnEnd' ? `<button class="turn-receipt ${game.result.busted ? 'bust-receipt' : ''}" data-action="next" aria-label="${game.result.busted ? 'Bust' : 'Collected'}. Continue to the next turn."><span>${game.result.busted ? 'BUST' : 'COLLECTED'}</span><b>+${game.result.delta} pts${game.result.bonus.length ? ` · ${game.result.bonus.length} bonus cards` : ''}${game.result.locker ? ` · ${game.result.lost.length} to the Locker` : ''}</b><small>${human ? 'Rook takes the helm' : 'Your turn'}<i class="turn-progress ${fast ? 'quick' : ''}"></i></small></button>`
    : !human ? `<div class="bot-working"><span class="thinking-dots"><i></i><i></i><i></i></span><span>Captain Rook is weighing the odds</span><button class="text-button" data-action="speed">${fast ? '1×' : '2×'}</button></div>`
    : bankChoice ? `<div class="choice-reminder">${dragCopy[game.choice.type]}</div>`
    : pickable.length ? '<div class="choice-reminder">Tap a card in the haul above.</div>'
    : choice ? `<div class="choice-reminder floating">Choose one of the cards ${options(game).length > 3 ? 'in the strip below' : 'floating on the right'}.</div>`
    : `<button class="primary collect-button" data-focus="collect" data-action="collect" ${!game.play.length || game.forced && game.deck.length ? 'disabled' : ''}><span>Collect<small>${game.forced ? 'Kraken must resolve' : game.play.length ? `${game.play.length} cards · +${preview} pts${combo ? ' + bonus' : ''}` : 'Draw to begin'}</small></span><span>→</span></button>`;
  const mapChoice = choice && zone?.zone === 'panel';
  $('#app').innerHTML = `<div class="shell game-shell">${topbar()}<div class="scoreboard"><div class="score-panel ${human && playing ? 'at-helm' : ''}"><div><span class="eyebrow">YOU</span><small>${human && playing ? 'At the helm' : 'Your bank'}</small></div><strong>${scoreOf(game, 0)}<small>pts</small></strong></div><span class="versus">vs</span><div class="score-panel ${!human && playing ? 'at-helm rival' : ''}"><div><span class="eyebrow">CAPTAIN ROOK</span><small>${!human && playing ? 'At the helm' : 'Bot captain'}</small></div><strong>${scoreOf(game, 1)}<small>pts</small></strong></div></div>${bankMarkup(1)}<section class="turn-status ${game.result?.busted ? 'bust-status' : ''} ${playDrop ? 'drag-target' : ''}" ${playDrop ? 'data-drop-target="play"' : ''} aria-label="Turn instructions"><h2>${title}</h2><p>${esc(subtitle)}</p></section><section class="table ${playDrop ? 'drag-target' : ''}" ${playDrop ? `data-drop-target="play" aria-label="${dropLabel[game.choice.type]}; ${dragCopy[game.choice.type]}"` : 'aria-label="Cards in play"'}><div class="table-topline"><span class="eyebrow">TURN ${game.turn} <span class="turn-dot ${human ? '' : 'rival-dot'}"></span> ${human ? 'YOUR' : 'ROOK’S'} HAUL</span><span class="pile-count"><b>${game.deck.length}</b> draw <span>·</span> <b>${game.discard.length}</b> discard</span></div><button class="draw-pile" data-focus="draw" data-action="draw" ${deckDisabled ? 'disabled' : ''} aria-label="${game.deck.length ? `Draw the next card. ${game.deck.length} cards left in the deck.` : 'The draw deck is empty'}"><strong>${game.deck.length}</strong></button><div class="play-grid ${!cards.length ? 'empty-play' : ''}" style="--play-count:${cards.length}">${cards.length ? cards.map((c, i) => smallCard(c, { action: pickable.includes(c.id) ? 'choose' : 'suit', target: pickable.includes(c.id), caption: pickable.includes(c.id) ? replayCaption : '', protectedCard: safe.has(c.id), danger: Boolean(game.result?.busted && i === cards.length - 1) })).join('') : `<div class="empty-sea">${icon('compass')}<span>Every fortune starts<br>with a little courage.</span></div>`}</div><div class="table-info">${info}</div></section>${cannonDrop ? `<div class="cannon-drop" data-drop-target="discard" role="status" aria-label="Swipe a highlighted card here to discard it"><span class="instruction-icon">${icon('cannon')}</span><b>Swipe to sink</b></div>` : ''}${mapChoice ? `<section class="choice-panel ${options(game).length > 3 ? 'wide' : ''}" aria-label="${game.choice.type} card choices"><div class="choice-grid">${options(game).map(c => smallCard(c, { action: 'choose', danger: duplicate(game, c), caption: duplicate(game, c) ? 'Will bust' : 'Play card' })).join('')}</div></section>` : ''}${bankMarkup(0)}<div class="bottom-space"></div><footer class="action-dock">${dock}</footer>${saveFailed ? '<p class="storage-warning">Saving is unavailable in this browser. Keep this tab open to finish your voyage.</p>' : ''}</div>`;
  if (focus) $(`[data-focus="${focus}"]`)?.focus({ preventScroll: true });
  $('#announcer').textContent = `${title}. ${subtitle}`;
  scheduleTurn();
}
// The table hands itself back and forth: a finished turn rolls into the next one,
// and Rook plays on its own timer, so the only taps needed are the player's moves.
function scheduleTurn() {
  clearTimeout(timer);
  if (screen !== 'game' || !game || $('#sheet').open) return;
  if (game.phase === 'turnEnd') timer = setTimeout(() => move({ type: 'next' }), fast ? 900 : 1800);
  else if (game.active === 1 && game.phase === 'play') timer = setTimeout(() => move(botAction(game)), fast ? 330 : 1100);
}
function move(action) {
  const wasChoice = Boolean(game.choice);
  if (!act(game, action)) return;
  persist(); render();
  cancelAnimationFrame(scrollFrame);
  scrollFrame = requestAnimationFrame(() => {
    // A pending drag is useless off screen, so the hand it comes from leads.
    // Map choices float beside the table, so only a handover needs the table itself.
    const source = $('[data-drag-card="true"]');
    if (source) source.closest('.bank')?.scrollIntoView({ block: 'center', behavior: 'instant' });
    else if (wasChoice || action.type === 'next') $('.table')?.scrollIntoView({ block: 'nearest', behavior: 'smooth' });
  });
  if (game.phase === 'over') showResults();
}
let returnFocus;
function modal(title, content, cls = '') {
  clearTimeout(timer);
  const sheet = $('#sheet');
  if (!sheet.open) returnFocus = document.activeElement;
  sheet.className = cls;
  sheet.innerHTML = `<div class="sheet-header"><h2 id="sheet-title">${title}</h2><button class="round-button" data-action="close" aria-label="Close dialog">×</button></div>${content}`;
  if (!sheet.open) sheet.showModal();
  sheet.scrollTop = 0;
}
function closeModal() { $('#sheet').close(); }
$('#sheet').addEventListener('close', () => { if (returnFocus?.isConnected) returnFocus.focus(); scheduleTurn(); });
$('#sheet').addEventListener('click', e => { if (e.target === $('#sheet')) { const rect = e.target.getBoundingClientRect(); if (e.clientX < rect.left || e.clientX > rect.right || e.clientY < rect.top || e.clientY > rect.bottom) closeModal(); } });

function help() {
  // The guide reads for whatever is on the table: a live game, or the toggles at home.
  const v = game?.variants || variants;
  modal('Learn the ropes', `<div class="help-intro">The richest captain wins.<br><span>Only your highest card in each suit scores.</span></div><ol class="help-steps"><li><b>Draw treasure</b><p>Click the deck to draw a card. Its ability activates immediately. Resolve it before doing anything else.</p></li><li><b>Keep your nerve</b><p>Keep drawing different suits to grow your haul. Repeat any suit in play and you bust: discard the haul, except cards protected by an Anchor.</p></li><li><b>Know when to leave</b><p>Collect to move the whole haul into your hand. The table then passes to your rival on its own. The highest card of each suit scores; lower cards stay underneath.</p></li></ol><div class="rules-note"><b>The last card</b><p>Finish every required ability on the final draw, then collect or bust. Highest score wins. Ties go to the captain with more banked cards, then a shared victory.</p></div><h3 class="guide-title">Know your ten suits</h3><div class="suit-guide">${SUITS.map(s => { const copy = suitCopy(s, v); return `<article style="--suit:${s.color}">${icon(s.id)}<div><h3>${s.name}</h3><p>${copy.rule}</p></div></article>`; }).join('')}</div><div class="rules-note"><b>A few finer points</b><p>Abilities trigger whenever a card enters play, including stolen or recovered cards. Hook and Sword are resolved by dragging a highlighted card from a hand anywhere onto the table; a Cannon fires when you swipe the card out to the right. A card that causes a bust never activates. If there is no legal target, skip that ability. When an Anchor saves both a Key and Chest, those protected cards earn the bonus, too.</p><p>The 60-card deck has six cards per suit. The lowest of each suit starts in the discard; shuffle the other 50 to draw from. The starting captain is chosen at random.</p><p>Tap any suit to read its ability. Tap a bank to inspect all its cards. Your voyage saves after every move.</p></div><h3 class="guide-title">Optional variants</h3><p class="muted">Turn either on from the title screen before a new voyage. ${v.traits || v.mermaid ? `This voyage sails with <b>${VARIANTS.filter(x => v[x.id]).map(x => x.name).join(' and ')}</b>.` : 'This voyage sails with the base rules.'}</p><div class="variant-guide">${VARIANTS.map(x => `<article style="--suit:${SUITS.find(s => s.id === x.suit).color}">${icon(x.suit)}<div><h3>${x.name} ${v[x.id] ? '<em>· on</em>' : ''}</h3><p>${x.blurb}</p></div></article>`).join('')}</div>${v.mermaid ? '<div class="rules-note"><b>Playing the Mermaid variant</b><p>The Mermaid 8 and 9 leave the deck and the Mermaid 2 and 3 join it, so every suit runs 2–7. A Mermaid entering your play area must replay an earlier card: it moves to the right of the Mermaid and its ability fires again. Move an Anchor that way and it protects everything now sitting to its left. A Mermaid played as the first card of a haul has nothing to replay, so its ability is skipped.</p></div>' : ''}${v.traits ? `<h3 class="guide-title">The trait deck</h3><p class="muted">Two traits are dealt to each captain; each keeps one, and both are revealed. Traits are always active and always apply when they can.</p><div class="trait-guide">${traitPool(v.mermaid).map(t => `<article style="--suit:${SUITS.find(s => s.id === t.suit).color}">${icon(t.suit)}<div><h3>${t.name}</h3><p>${traitCopy(t, v)}</p></div></article>`).join('')}</div>${v.mermaid ? '<div class="rules-note"><b>Siren and Casanova</b><p>Both traits snatch the card a Mermaid selects before its ability can fire. If both are in play, the Siren steals first. When a banked Anchor leaves the table this way, everything it was already protecting stays protected.</p></div>' : ''}` : ''}<button class="primary full" data-action="close">Ready, Captain</button>`);
}
function showSuit(id, n) {
  const s = SUITS.find(s => s.id === id); if (!s) return;
  const cards = n === undefined ? [] : stack(game.banks[n], id);
  modal(s.name, `<div class="suit-detail" style="--suit:${s.color}">${icon(id)}<p>${suitCopy(s, game?.variants).rule}</p></div>${n !== undefined ? `<div class="rules-note"><b>${n === 0 ? 'Your' : 'Rook’s'} ${s.name} stack</b><p>${cards.length ? `${cards.map(c => c.value).join(' · ')} — the highest card scores ${cards[0].value} points.${game.traits?.[n] === 'golden-scales' && id === 'mermaid' ? ' Golden Scales adds five more.' : ''}` : 'No cards collected in this suit yet.'}</p></div>` : ''}<button class="primary full" data-action="close">Back to the voyage</button>`);
}
function showTrait(id) {
  const t = traitOf(id); if (!t) return;
  const owner = game?.traits?.indexOf(id) ?? -1;
  modal(t.name, `<div class="suit-detail" style="--suit:${SUITS.find(s => s.id === t.suit).color}">${icon(t.suit)}<p>${traitCopy(t, game?.variants)}</p></div><div class="rules-note"><b>${owner === 0 ? 'Your trait' : owner === 1 ? 'Captain Rook’s trait' : 'Trait card'}</b><p>Traits stay active for the whole voyage and must be applied whenever they can be.</p></div><button class="primary full" data-action="close">Back to the voyage</button>`);
}
function showBank(n) {
  modal(n === 0 ? 'Your treasure' : 'Rook’s treasure', `<div class="bank-total"><strong>${scoreOf(game, n)}</strong><span>points across ${game.banks[n].length} cards</span></div><div class="bank-list">${SUITS.map(s => { const cards = stack(game.banks[n], s.id); return `<div style="--suit:${s.color}">${icon(s.id)}<span>${s.name}<small>${cards.map(c => c.value).join(' · ') || 'No treasure yet'}</small></span><b>${cards[0]?.value || '–'}</b></div>`; }).join('')}</div><p class="muted">Each suit’s highest card scores. The rest remain in your bank, ready to be exposed or replayed.</p>`);
}
function showResults() {
  const a = scoreOf(game, 0), b = scoreOf(game, 1);
  const bonusRow = n => game.traits?.[n] === 'golden-scales' && stack(game.banks[n], 'mermaid')[0] ? ' +5' : '';
  modal(game.winner === 0 ? 'The sea is yours.' : game.winner === 1 ? 'Rook takes the riches.' : 'A fortune shared.', `<div class="result-hero">${icon(game.winner === 0 ? 'chest' : game.winner === 1 ? 'ship' : 'anchor')}<p>${game.winner === 0 ? 'Well sailed, Captain.' : game.winner === 1 ? 'There’s always another horizon.' : 'Two captains. Equal treasure.'}</p></div><div class="final-scores"><div class="${game.winner === 0 ? 'winner' : ''}"><span>YOU</span><strong>${a}</strong><small>${game.banks[0].length} cards</small></div><div class="${game.winner === 1 ? 'winner' : ''}"><span>CAPTAIN ROOK</span><strong>${b}</strong><small>${game.banks[1].length} cards</small></div></div>${game.variants?.traits ? traitBar() : ''}${a === b ? `<p class="rules-note">${game.winner === -1 ? 'Scores and card counts are equal. You share the victory.' : 'Scores tied. The captain with more banked cards wins.'}</p>` : ''}<div class="score-breakdown"><div class="breakdown-heading"><span>SUIT</span><b>YOU</b><b>ROOK</b></div>${SUITS.map(s => `<div style="--suit:${s.color}"><span>${icon(s.id)}${s.name}</span><b>${stack(game.banks[0], s.id)[0] ? stack(game.banks[0], s.id)[0].value + (s.id === 'mermaid' ? bonusRow(0) : '') : '–'}</b><b>${stack(game.banks[1], s.id)[0] ? stack(game.banks[1], s.id)[0].value + (s.id === 'mermaid' ? bonusRow(1) : '') : '–'}</b></div>`).join('')}</div><p class="muted">${esc(game.message)}</p><button class="primary full" data-action="start">Sail again <span>→</span></button><button class="text-button full" data-action="close">Inspect the final table</button>`, 'results-sheet');
}
function activeVariants(v) { const on = VARIANTS.filter(x => v?.[x.id]); return on.length ? on.map(x => x.name).join(' + ') : 'Base rules'; }
function menu() {
  modal('Captain’s quarters', `<div class="menu-list"><button data-action="help">Learn the ropes <span>→</span></button><button data-action="log">Voyage log <span>→</span></button><button data-action="speed">Bot pace <span>${fast ? 'Quick' : 'Leisurely'}</span></button><button data-action="restart">New voyage <span>↗</span></button><button data-action="home">Return to title <span>→</span></button></div><p class="muted">Sailing with: <b>${activeVariants(game?.variants)}</b>. Variants are chosen on the title screen before a new voyage.</p><p class="muted">${saveFailed ? 'Saving is unavailable. Keep this tab open.' : 'Your voyage is saved automatically on this device.'}</p>`);
}
function start() { clearTimeout(timer); closeModal(); game = newGame(Math.random, variants); persist(); render(); }
function toggleVariant(id) {
  variants = { ...variants, [id]: !variants[id] };
  try { localStorage.setItem(VARIANT_KEY, JSON.stringify(variants)); } catch { /* Playing without storage still works. */ }
  home();
}
let dragState = null, dragGhost = null, suppressClick = false;
function clearDragPreview() {
  document.querySelectorAll('.dragging').forEach(el => { el.classList.remove('dragging'); el.setAttribute('aria-grabbed', 'false'); });
  document.querySelectorAll('.drop-hover').forEach(el => el.classList.remove('drop-hover'));
  document.body.classList.remove('drop-armed');
  dragGhost?.remove(); dragGhost = null;
}
const inside = (el, x, y) => { const r = el.getBoundingClientRect(); return x >= r.left && x <= r.right && y >= r.top && y <= r.bottom; };
// Dropping is forgiving on purpose: the whole table takes a replayed card, and a
// cannon fires on any long swipe to the right, not just a hit on the rail.
function dropZones(x, y) {
  if (!dragState || !game?.choice) return [];
  const cannon = game.choice.type === 'cannon';
  const zones = [...document.querySelectorAll(`[data-drop-target="${cannon ? 'discard' : 'play'}"]`)];
  if (cannon && x - dragState.startX > Math.min(150, innerWidth * .22)) return zones;
  return zones.filter(el => inside(el, x, y));
}
document.addEventListener('pointerdown', e => {
  if ((e.pointerType === 'mouse' && e.button !== 0) || !game?.choice) return;
  const source = e.target.closest('[data-drag-card="true"]');
  if (!source || !options(game).some(c => c.id === source.dataset.id)) return;
  dragState = { id: source.dataset.id, source, pointerId: e.pointerId, startX: e.clientX, startY: e.clientY, moved: false };
  try { source.setPointerCapture(e.pointerId); } catch { /* Capture is a nicety; the document listeners still see the drag. */ }
});
document.addEventListener('pointermove', e => {
  if (!dragState || e.pointerId !== dragState.pointerId) return;
  const distance = Math.hypot(e.clientX - dragState.startX, e.clientY - dragState.startY);
  if (!dragState.moved && distance < 6) return;
  e.preventDefault();
  if (!dragState.moved) {
    dragState.moved = true;
    dragState.source.classList.add('dragging');
    dragState.source.setAttribute('aria-grabbed', 'true');
    dragGhost = dragState.source.cloneNode(true);
    dragGhost.classList.add('drag-ghost');
    dragGhost.style.width = `${dragState.source.getBoundingClientRect().width}px`;
    document.body.append(dragGhost);
  }
  dragGhost.style.transform = `translate(${e.clientX + 12}px, ${e.clientY + 12}px)`;
  const armed = dropZones(e.clientX, e.clientY);
  document.querySelectorAll('[data-drop-target].drop-hover').forEach(el => { if (!armed.includes(el)) el.classList.remove('drop-hover'); });
  armed.forEach(el => el.classList.add('drop-hover'));
  document.body.classList.toggle('drop-armed', armed.length > 0);
}, { passive: false });
function endDrag(e) {
  if (!dragState || e.pointerId !== dragState.pointerId) return;
  const { id, moved } = dragState;
  const dropped = moved && dropZones(e.clientX, e.clientY).length > 0;
  clearDragPreview(); dragState = null;
  if (!moved) return;
  suppressClick = true;
  setTimeout(() => { suppressClick = false; }, 0);
  if (dropped) move({ type: 'choose', id });
}
document.addEventListener('pointerup', endDrag);
document.addEventListener('pointercancel', e => { if (dragState?.pointerId === e.pointerId) { clearDragPreview(); dragState = null; } });
document.addEventListener('click', e => {
  if (suppressClick) return;
  const b = e.target.closest('[data-action]'); if (!b || b.disabled) return;
  const action = b.dataset.action;
  if (action === 'close') closeModal();
  else if (action === 'help') help();
  else if (action === 'start') start();
  else if (action === 'resume') { game = saved; render(); }
  else if (action === 'variant') toggleVariant(b.dataset.variant);
  else if (action === 'trait-info') showTrait(b.dataset.id);
  else if (action === 'trait') move({ type: 'trait', id: b.dataset.id });
  else if (action === 'restart') modal('Start a new voyage?', `<p class="confirm-copy">Your current treasure and progress will be replaced by a freshly shuffled deck, sailing with <b>${activeVariants(variants)}</b>.</p><button class="primary full" data-action="start">Start new voyage</button><button class="secondary full" data-action="close">Keep this voyage</button>`);
  else if (action === 'home') { closeModal(); clearTimeout(timer); home(); }
  else if (action === 'menu') menu();
  else if (action === 'results') showResults();
  else if (action === 'bank') showBank(Number(b.dataset.player));
  else if (action === 'suit') showSuit(b.dataset.suit);
  else if (action === 'stack') showSuit(b.dataset.suit, Number(b.dataset.player));
  else if (action === 'speed') { fast = !fast; try { localStorage.setItem('pirates.fast', fast); } catch {} if ($('#sheet').open) menu(); else render(); }
  else if (action === 'log') modal('Voyage log', `<div class="voyage-log">${game.history.length ? game.history.map(h => `<article><span>TURN ${h.turn} · ${h.player === 0 ? 'YOU' : 'ROOK'}</span><p>${esc(h.text)}</p></article>`).join('') : '<p>Your story is still unwritten. Draw your first card.</p>'}</div>`);
  else if (game && (game.active === 0 || action === 'next')) {
    if (['draw', 'collect', 'next'].includes(action)) move({ type: action });
    else if (action === 'choose') move({ type: 'choose', id: b.dataset.id });
  }
});
document.addEventListener('visibilitychange', () => { if (document.hidden) clearTimeout(timer); else scheduleTurn(); });
home();
